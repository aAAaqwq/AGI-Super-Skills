...
|---|---|---|---|
| 🤖 AI Agents & Orchestration | [`dispatching-parallel-agents`](../skills/dispatching-parallel-agents/) | Harness-specific assignment · generic installer skips | Break independent work into bounded parallel tasks. |
...
| ⚙️ Apps & Workflow Automation | [`github-automation`](../skills/github-automation/) | Portable required assignment · structure checked · review: credentials, external-write, system-change | Work with GitHub through an explicit automation boundary. |
...
| [`agent-skills-audit`](../skills/agent-skills-audit/) | Run a two-pass, multidisciplinary code audit led by a tie-breaker lead, combining security, performance, UX, DX, and edge-case analysis into one prioritized report with concrete… | Portable required assignment · structure checked · GOVERNOR |
...
| [`context-manager`](../skills/context-manager/) | AI-powered context management for OpenClaw sessions | Harness-specific assignment · generic installer skips · CEO |
...
| [`self-improving`](../skills/self-improving/) | Self-reflection + Self-criticism + Self-learning + Self-organizing memory. Agent evaluates its own work, catches mistakes, and improves permanently. Use before starting work and… | Harness-specific assignment · generic installer skips · CPO, CSO |
...
| [`architecture-decision`](../skills/architecture-decision/) | Systematically evaluate architecture decisions, document trade-offs, and select appropriate patterns. This skill should be used when the user asks about 'architecture decision',… | Portable required assignment · structure checked · CTO |
...
| [`architecture-patterns`](../skills/architecture-patterns/) | Software architecture patterns and best practices | Portable required assignment · structure checked · CTO |
...
| [`claude-code-runner`](../skills/claude-code-runner/) | Execute programming tasks via Claude Code using PTY-based invocation. Handles non-TTY environments, auto-responds to prompts, and manages file synchronization. | Harness-specific assignment · generic installer skips · PE |
| [`cli-developer`](../skills/cli-developer/) | Use when building CLI tools, implementing argument parsing, or adding interactive prompts. Invoke for parsing flags and subcommands, displaying progress bars and spinners,… | Portable required assignment · structure checked · PE |
...
| [`finishing-a-development-branch`](../skills/finishing-a-development-branch/) | Use when implementation is complete, all tests pass, and you need to decide how to integrate the work - guides completion of development work by presenting structured options for… | Portable required assignment · structure checked · PE |
| [`gh-issues`](../skills/gh-issues/) | Fetch GitHub issues, spawn sub-agents to implement fixes and open PRs, then monitor and address PR review comments. Usage: /gh-issues [owner/repo] [--label bug] [--limit 5]… | Harness-specific assignment · generic installer skips · PE |
...
| [`openai-whisper`](../skills/openai-whisper/) | Local speech-to-text with the Whisper CLI (no API key). | Portable required assignment · structure checked · CCO |
...
| [`react-architect`](../skills/react-architect/) | Build production-grade React applications with modern architecture, state management, and performance optimization. Use when: (1) creating new React projects, (2) component… | Portable required assignment · structure checked · PE |
...
| [`react-expert`](../skills/react-expert/) | Use when researching React APIs or concepts for documentation. Use when you need authoritative usage examples, caveats, warnings, or errors for a React feature. | Portable required assignment · structure checked · CEO, PE |
...
| [`systematic-debugging`](../skills/systematic-debugging/) | Use when encountering any bug, test failure, or unexpected behavior, before proposing fixes | Harness-specific assignment · generic installer skips · CEO, PE |
...
| [`tdd-workflow`](../skills/tdd-workflow/) | Use this skill when writing new features, fixing bugs, or refactoring code. Enforces test-driven development with 80%+ coverage including unit, integration, and E2E tests. | Portable required assignment · structure checked · CEO, PE |
...
| [`docker-essentials`](../skills/docker-essentials/) | Essential Docker commands and workflows for container management, image operations, and debugging. | Portable required assignment · structure checked · CTO, PE |
...
| [`healthcheck`](../skills/healthcheck/) | Host security hardening and risk-tolerance configuration for OpenClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure… | Harness-specific assignment · generic installer skips · CEO, COO |
...
| [`render-automation`](../skills/render-automation/) | Automate Render tasks via Rube MCP (Composio): services, deployments, projects. Always search tools first for current schemas. | Portable required assignment · structure checked · CTO |
...
| [`sentry-automation`](../skills/sentry-automation/) | Automate Sentry tasks via Rube MCP (Composio): manage issues/events, configure alerts, track releases, monitor projects and teams. Always search tools first for current schemas. | Portable required assignment · structure checked · CTO |
...
| [`vercel-automation`](../skills/vercel-automation/) | Automate Vercel tasks via Rube MCP (Composio): manage deployments, domains, DNS, env vars, projects, and teams. Always search tools first for current schemas. | Portable required assignment · structure checked · CTO |
...
| [`google-analytics`](../skills/google-analytics/) | Analyze Google Analytics data, review website performance metrics, identify traffic patterns, and suggest data-driven improvements. Use when the user asks about analytics,… | Portable required assignment · structure checked · CEO |
...
| [`auth-manager`](../skills/auth-manager/) | 网页登录态管理。使用 OpenClaw 内置 browser (profile=openclaw) 统一管理各平台登录状态，定期检查可用性。 | Harness-specific assignment · generic installer skips · CTO |
| [`auth-system`](../skills/auth-system/) | Implement secure authentication and authorization systems with JWT, OAuth2, Session-based auth, and RBAC. Use when: (1) implementing user login/signup, (2) setting up JWT tokens,… | Portable required assignment · structure checked · PE |
...
| [`contract-review`](../skills/contract-review/) | Legal contract analysis using CUAD dataset (41 risk categories). Supports NDA, SaaS, M&amp;A, employment, payment/merchant, and finder/broker agreements. Identifies red flags,… | Portable required assignment · structure checked · CEO |
...
| [`legal-review`](../skills/legal-review/) | Review legal documents (NDA, contracts, agreements) for sensitive clauses, risks, and red flags | Portable required assignment · structure checked · CLO |
| [`legal-risk-assessment`](../skills/legal-risk-assessment/) | Structured legal risk assessment with 5x5 Severity x Likelihood matrix. Use for risk scoring, risk registers, escalation decisions, and risk memos. | Portable required assignment · structure checked · CEO |
...
| [`user-story`](../skills/user-story/) | Create clear, concise user stories that combine Mike Cohn's user story format with Gherkin-style acceptance criteria. Use this to translate user needs into actionable development… | Portable required assignment · structure checked · CEO, CPO |
...
| [`brand-identity`](../skills/brand-identity/) | Build a complete brand identity for a solopreneur business from scratch or refresh an existing one. Covers brand personality, voice and tone, visual identity system (colors,… | Portable required assignment · structure checked · CPO |
...
| [`competitive-analysis`](../skills/competitive-analysis/) | Help users understand and respond to competition. Use when someone is positioning against competitors, evaluating market threats, running competitive war games, or deciding how… | Portable required assignment · structure checked · CEO |
...
| [`competitor-alternatives`](../skills/competitor-alternatives/) | When the user wants to create competitor comparison or alternative pages for SEO and sales enablement. Also use when the user mentions 'alternative page,' 'vs page,' 'competitor… | Portable required assignment · structure checked · CMO, CSO |
...
| [`traffic-acquisition`](../skills/traffic-acquisition/) | Use when driving traffic to Xiaohongshu account from external sources, acquiring new followers beyond organic discovery, implementing multi-platform growth strategy, or scaling… | Portable required assignment · structure checked · CEO |
| [`video-marketing`](../skills/video-marketing/) | When the user wants to plan video marketing, create video scripts, or optimize for short-form or long-form video. Also use when the user mentions "video marketing," "video… | Portable required assignment · structure checked · CEO |
...
| [`content-cover-gen`](../skills/content-cover-gen/) | 内容驱动封面生成:从文章标题/核心观点自动生成视觉隐喻提示词,调用 relay-image-gen 出图。 每张封面3秒内传达文章核心观点,杜绝通用背景。 支持小红书(3:4)、抖音(9:16)、公众号(16:9)、知识星球(1:1)、掘金(16:9)。 触发:'生成封面'、'封面生成'、'cover'、'thumbnail'、'封面图'、'缩略图'。 | Harness-specific assignment · generic installer skips · CCO |
...
| [`content-ops-toolkit`](../skills/content-ops-toolkit/) | Analyze topics, score headlines, adapt drafts across platforms, and review content performance. Use for 选题分析、标题优化、多平台适配、内容复盘, or content review. | Harness-specific assignment · generic installer skips · CEO |
...
| [`nano-banana-pro`](../skills/nano-banana-pro/) | Generate/edit images with Nano Banana Pro (Gemini 3 Pro Image). Use for image create/modify requests incl. edits. Supports text-to-image + image-to-image; 1K/2K/4K; use… | Harness-specific assignment · generic installer skips · CCO |
...
| [`video-merge-send`](../skills/video-merge-send/) | 合并多个分段视频为一个完整视频，并发送到飞书。使用ffmpeg拼接视频片段，支持淡入淡出转场。触发场景：用户需要合并视频片段、拼接分镜视频、视频合并后发飞书、把多个短视频合成一个、分段视频合并发送。配合 jimeng-storyboard skill 使用，完成数字人视频全流程。 | Portable required assignment · structure checked · CCO |
...
| [`cold-email-sequence-generator`](../skills/cold-email-sequence-generator/) | Generate personalized cold email sequences (7-14 emails) with A/B test subject lines, follow-up timing recommendations, and integrated social proof. Creates multi-touch campaigns… | Portable required assignment · structure checked · CEO |
| [`crm-automation`](../skills/crm-automation/) | CRM workflow automation for HubSpot, Salesforce, Pipedrive - lead management, deal tracking, and multi-CRM synchronization | Portable required assignment · structure checked · CSO |
...
| [`backtest-expert`](../skills/backtest-expert/) | Expert guidance for systematic backtesting of trading strategies. Use when developing, testing, stress-testing, or validating quantitative trading strategies. Covers "beating… | Portable required assignment · structure checked · CEO |
...
| [`financial-analyst`](../skills/financial-analyst/) | Performs financial ratio analysis, DCF valuation, budget variance analysis, and rolling forecast construction for strategic decision-making | Portable required assignment · structure checked · CEO |
...
| [`polymarket-api`](../skills/polymarket-api/) | Polymarket交易操作。统一入口scripts/trade.py，slug驱动，零browser纯API。Triggers: 'polymarket交易', 'API下单', 'buy YES', 'sell NO', 'trade', 'price', '仓位'. | Harness-specific assignment · generic installer skips · CEO, CQO |
| [`polymarket-copy-tracker`](../skills/polymarket-copy-tracker/) | Analyze top Polymarket traders for copy trading. Use when asked to find best traders to copy, analyze trader performance/history, evaluate wallet PnL, discover high-ROI wallets,… | Portable required assignment · structure checked · CQO |
...
| [`project-planner`](../skills/project-planner/) | 项目路径规划与执行助手。帮助分析项目需求、规划执行路径、分解任务、识别风险、管理进度，确保项目高质量交付。 | Portable required assignment · structure checked · CEO |
...
| [`browser-profile-guide`](../skills/browser-profile-guide/) | OpenClaw Browser 配置文件系统完全指南 — 哪个 profile 何时用、如何选 | Harness-specific assignment · generic installer skips · COO, CTO, PE |
| [`browser-use`](../skills/browser-use/) | AI驱动的智能浏览器自动化工具。使用LLM理解页面并自动执行任务，比传统Playwright更智能、更省token。适用于复杂交互、动态页面、需要智能决策的浏览器操作。Chrome浏览器优先。 | Harness-specific assignment · generic installer skips · CTO |
...
| [`douyin-publisher`](../skills/douyin-publisher/) | 抖音图文/视频发布（OpenClaw Browser 自动化）：登录检测→内容校验→上传页导航→填标题/正文→上传封面+素材→暂存离开。 | Harness-specific assignment · generic installer skips · CCO · review: credentials, external-write |
| [`douyin-smart-publish`](../skills/douyin-smart-publish/) | 抖音创作者平台智能发布（视频/图文）：内容适配→上传→填描述/#话题→封面→存草稿→截图回传确认。默认只存草稿；只有在 Daniel 明确确认后才允许点击发布。覆盖标题(≤55字)、描述(建议≤200字)、#话题标签(3-5个)、封面设置、定时发布、可见性、合拍/下载开关。Playwright 自动化。触发：'发抖音'、'抖音发布'、'douyin… | Harness-specific assignment · generic installer skips · CCO |
...
| [`wechat-toolkit`](../skills/wechat-toolkit/) | 微信公众号一站式工具包 — 集成文章搜索、文章下载、AI洗稿改写、公众号发布四大功能。当用户需要搜索/下载/改写/发布微信公众号文章时使用。 | Harness-specific assignment · generic installer skips · CCO |
...
| [`xhs-publisher`](../skills/xhs-publisher/) | 将 Markdown 文章自动发布到小红书（XHS）草稿箱。支持多图上传（封面+素材）。 | Harness-specific assignment · generic installer skips · CCO · review: credentials, external-write |
...
| [`xiaohongshu-growth`](../skills/xiaohongshu-growth/) | 小红书内容创作运营增长综合解决方案。包含爆款内容创作、封面设计、发布优化、数据分析和账号增长策略。当用户需要：(1) 创建小红书爆款内容 (2) 优化账号运营策略 (3) 分析竞品和热点 (4) 发布内容到草稿箱 (5) 提升账号流量和变现能力时触发。支持完整的内容创作流水线和多agent协作。 | Portable required assignment · structure checked · CCO, CEO |
