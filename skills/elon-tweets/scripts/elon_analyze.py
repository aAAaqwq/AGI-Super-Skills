...
def analyze(json_path):
    ...
    # Get event info from root level
    event_end = datetime.fromisoformat(data['endDate'].replace('Z','+00:00'))
    event_start = datetime.fromisoformat(data['startDate'].replace('Z','+00:00'))
    now = datetime.now(timezone.utc)
    remain = (event_end - now).total_seconds() / 3600
    total = (event_end - event_start).total_seconds() / 3600
    days = total / 24

    print(f"Period: {event_start.strftime('%b%d')}→{event_end.strftime('%b%d %H:%M')}ET | {remain:.1f}h left | {days:.1f}d")

    intervals = []
    for m in data['markets']:
        q = m['question']
        try: p = m['outcomePrices']  # Already a list, no need for json.loads
        except: continue
