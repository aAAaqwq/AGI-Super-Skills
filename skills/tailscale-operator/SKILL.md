...
description: Manage, connect to, and troubleshoot devices on a Tailscale tailnet.
  Use when a user asks to connect to a device over Tailscale, find a device IP, SSH
  into a tailnet node, fix SSH auth failures, set up public-key login, enable Tailscale
  SSH, diagnose permission or sudo issues, or operate remote Macs/Linux hosts over
  the tailnet.
author: Daniel Li
---
...
Check these first:
- Shared tailnet inventory: `<shared-tailnet-readme>`
- Device records: `<shared-tailnet-device-records>`
- Local notes: `<local-tools-notes>`
