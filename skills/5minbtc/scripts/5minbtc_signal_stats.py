...
def fmt_report(state, by_hour=False):
    ...
    lines.append("━" * 26)
    lines.append(f"信号 {len(auto)} | 已结算 {len(settled)} | 待结算 {len(open_)}")
    if not settled:
    ...
    lines.append(f"胜率 {wins}/{len(settled)} = {wins/len(settled)*100:.0f}% | "
                 f"PnL ${pnl:+.2f}")
    ...
    for k in sorted(groups):
        ...
        avg_conf = sum(b.get("entry", {}).get("confidence", 0) for b in g) / len(g)
        avg_ask = sum(b.get("ask", 0) for b in g) / len(g)
        lines.append(f"{k}: {len(g)}笔 胜率{w/len(g)*100:.0f}% PnL${p:+.2f} "
                     f"(均conf{avg_conf:.0f} 均ask{avg_ask:.2f})")
    return "\n".join(lines)
