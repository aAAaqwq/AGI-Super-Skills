...
analyze_symbol() {
    ...
    # 24h ticker (try binance.vision first, fallback to binance.us)
    local T24=$(curl -s --max-time 6 "$BINANCE_API/ticker/24hr?symbol=$SYM" 2>/dev/null)
    if [ -z "$T24" ] || echo "$T24" | grep -q '"code"'; then
        T24=$(curl -s --max-time 6 "https://api.binance.us/api/v3/ticker/24hr?symbol=$SYM" 2>/dev/null)
    fi
    if [ -z "$T24" ] || echo "$T24" | grep -q '"code"'; then
        echo "$NAME: API_ERROR"
        return
    ...
    # 4h K线
    local K4H=$(curl -s --max-time 6 "$BINANCE_API/klines?symbol=$SYM&interval=4h&limit=4" 2>/dev/null)
    if [ -z "$K4H" ] || echo "$K4H" | grep -q '"code"'; then
        K4H=$(curl -s --max-time 6 "https://api.binance.us/api/v3/klines?symbol=$SYM&interval=4h&limit=4" 2>/dev/null)
    fi
